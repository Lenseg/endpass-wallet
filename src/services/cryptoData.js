...
const throttle = throttledQueue(3, process.env.VUE_APP_SERVICE_THROTTLE_TIMEOUT);
...
const cryptoDataService = {
  ...
  async getGasPrice(network) {
    try {
      const { data } = await http.get(
        `${process.env.VUE_APP_CRYPTODATA_API_URL}/${network}/gas/price`,
      );
  ...
  getSymbolsPrices(fromSymbols, toSymbol) {
    ...
    return new Promise((resolve, reject) => {
      ...
      throttle(async () => {
        try {
          const res = await http.get(`${process.env.VUE_APP_CRYPTODATA_API_URL}/price`, {
            params: {
    ...
    return new Promise((resolve, reject) => throttle(async () => {
      try {
        const res = await http.get(
          `${process.env.VUE_APP_CRYPTODATA_API_URL}/${network}/balance/${address}`,
        );
  ...
  getHistoryWithTokens({ network, address }) {
    return new Promise((resolve, reject) => {
      throttle(async () => {
        try {
          const res = await http.get(
            `${process.env.VUE_APP_CRYPTODATA_API_URL}/${network}/transactions/${address}/token`,
            {
              params: {
                page: 1,
                limit: 50,
  ...
  getHistory({ network, address }) {
    return new Promise((resolve, reject) => {
      throttle(async () => {
        try {
          const res = await http.get(
            `${process.env.VUE_APP_CRYPTODATA_API_URL}/${network}/transactions/${address}`,
            {
              params: {
                page: 1,
                limit: 50,
  ...
  async getPendingTransactions(network, fromAddress, filterId) {
    try {
      const { data } = await http.get(
        `${process.env.VUE_APP_CRYPTODATA_API_URL}/${network}/transactions/pending`,
        {
          params: {
            filterId,
            from: fromAddress,
